    ...
    fileHandler(req, res, cb){
        ...
        findfile.byPathname(this.base, pathname, { normalize : false, index : 'index.ejs' }, function(err, resolvedpathname, data){
            if(data){    
                ...
                ejs.renderFile(resolvedpathname, Object.assign(self.renderGlobalData, self.renderFileData[pathname] || {}, req.docs), self.options, function(err, str){
                    if(err){
                    throw new Error(err)
                    }
                    else{
